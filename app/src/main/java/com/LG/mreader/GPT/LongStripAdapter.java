...
public class LongStripAdapter extends RecyclerView.Adapter<ImageViewHolder> {
    ...
    public void onBindViewHolder(@NonNull ImageViewHolder holder, int position) {
        ...
        Uri uri = p.optimizedUri != null ? p.optimizedUri : p.sourceUri;
        holder.bind(uri);
        ...
        for (int i = 1; i <= prefetch; i++) {
            int idx = position + i;
            if (idx < pages.size()) {
                Uri nextUri = pages.get(idx).optimizedUri != null ? pages.get(idx).optimizedUri : pages.get(idx).sourceUri;
    ...
    public long getItemId(int position) {
        return pages.get(position).id.hashCode();
    }
