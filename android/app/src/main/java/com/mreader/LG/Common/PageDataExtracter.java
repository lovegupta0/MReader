...
public class PageDataExtracter {
    ...
    private static String extractMangaData(String html, String baseUrl) {
        ...
        // 2️⃣ Title (prefer <title>, fallback to <h1>, then clean up)
        String title = "";
        ...
        // Fix encoding issue for apostrophe
        title = title.replace('\u2019', '\'');

        // Clean up title
        if (title.startsWith("Manga: ")) {
            title = title.substring("Manga: ".length());
        }
        if (title.contains("Chapter - ")) {
            title = title.substring(0, title.indexOf("Chapter - ")).trim();
        }
    ...
    public static String fetchNextChapter(String urlString,String baseUrl){
        ...
        try {
            ...
            return extractMangaData(response.toString(),baseUrl);
